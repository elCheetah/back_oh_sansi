...
export async function importarDesdeArchivo(req: Request, res: Response) {
  try {
    if (!('files' in req) || !req.files || !(req.files as any).archivo) {
      return res
        .status(400)
        .json({ ok: false, mensaje: 'Debe enviar un archivo en el campo "archivo" (xlsx/xls/csv).' });
    }
    ...
    if (!esExtensionSoportada(archivo.name)) {
      return res
        .status(400)
        .json({ ok: false, mensaje: 'Formato no soportado. Use .xlsx, .xls o .csv.' });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .json({ ok: false, mensaje: 'No se pudo leer el archivo. Verifique el formato.' });
    }
    ...
    if (!filas.length) {
      return res
        .status(400)
        .json({ ok: false, mensaje: 'La primera hoja en Google Sheets está vacía.' });
    }
