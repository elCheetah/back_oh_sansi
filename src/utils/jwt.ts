...
export const TOKEN_TTL_SECONDS = 60 * 60 * 2; // 2h

export type Roles = "ADMINISTRADOR" | "EVALUADOR" | "RESPONSABLE";

export type AuthTokenPayload = {
  idUser: number;         // id de usuario (antes sub)
...
// Type guard para eliminar TS2352 y validar el payload
function isAuthPayload(x: unknown): x is AuthTokenPayload & JwtPayload {
  if (!x || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  const validRol = o.rol === "ADMINISTRADOR" || o.rol === "EVALUADOR" || o.rol === "RESPONSABLE";
  return (
